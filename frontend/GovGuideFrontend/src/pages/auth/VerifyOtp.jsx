...
export default function VerifyOtp() {
  ...
  const { verifyOtp, loading, error: authError, setError: setAuthError } = useAuth();
  ...
  const handleResendOtp = async () => {
    ...
    try {
      // API call to resend OTP when backend is ready
      ...
      <Card className="w-full max-w-md">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <Button
            ...
            type="button"
          >
            {resendCountdown > 0 ? `${t("auth.resendOtp")} (${resendCountdown}s)` : t("auth.resendOtp")}
          </Button>
        </form>
      </Card>
    </div>
